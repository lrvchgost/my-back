...
class ValidateDescriptionHasContentTest {
    @Test
    fun noContent() = runTest {
        val ctx = CatalogContext(state = CatalogState.RUNNING, storageValidating = Storage(description = "12!@#$%^&*()_+-="))
        chain.exec(ctx)
        assertEquals(CatalogState.FAILED, ctx.state)
        assertEquals(1, ctx.errors.size)
        assertEquals("validation-description-noContent", ctx.errors.first().code)
    }
